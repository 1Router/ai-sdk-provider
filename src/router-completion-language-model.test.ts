...
import { createrouter } from "./router-provider";
...
describe("doGenerate", () => {
  const server = new JsonTestServer("https://1router.com/api/v1/completions");
  ...
  it("should extract logprobs", async () => {
    ...
    const provider = createrouter({ apiKey: "test-api-key" });
    ...
    expect(response.logprobs).toStrictEqual(
      maprouterCompletionLogProbs(TEST_LOGPROBS)
    );
  });
...
describe("doStream", () => {
  const server = new StreamingTestServer(
    "https://1router.com/api/v1/completions"
  );
  ...
  it("should stream text deltas", async () => {
    ...
    expect(await convertReadableStreamToArray(stream)).toStrictEqual([
      ...
      { type: "text-delta", textDelta: "" },
      {
        type: "finish",
        finishReason: "stop",
        logprobs: maprouterCompletionLogProbs(TEST_LOGPROBS),
  ...
  it("should handle error stream parts", async () => {
    server.responseChunks = [
      `data: {"error":{"message": "The server had an error processing your request. Sorry about that! You can retry your request, or contact us through our ` +
        `help center at help.router.com if you keep seeing this error.","type":"server_error","param":null,"code":null}}\n\n`,
      "data: [DONE]\n\n",
    ];
    ...
    expect(await convertReadableStreamToArray(stream)).toStrictEqual([
      {
        ...
        error: {
          message:
            ...
            "help.router.com if you keep seeing this error.",
          type: "server_error",
          code: null,
          param: null,
        },
